...
const TIPS = [
  ...
  "Create the Klika Code config {highlight}opencode.json{/highlight} in project root for project-specific settings",
  "Place Klika Code settings in {highlight}~/.config/opencode/opencode.json{/highlight} for global config",
  ...
  "Klika Code auto-handles OAuth for remote MCP servers requiring auth",
  "Add {highlight}.md{/highlight} files to {highlight}.opencode/command/{/highlight} for Klika Code custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.opencode/agent/{/highlight} for Klika Code AI personas",
  "Configure per-agent permissions for {highlight}edit{/highlight}, {highlight}bash{/highlight}, and {highlight}webfetch{/highlight} tools",
  'Use patterns like {highlight}"git *": "allow"{/highlight} for granular bash permissions',
  'Set {highlight}"rm -rf *": "deny"{/highlight} to block destructive commands',
  'Configure {highlight}"git push": "ask"{/highlight} to require approval before pushing',
  "Klika Code auto-formats files using prettier, gofmt, ruff, and more",
  'Set {highlight}"formatter": false{/highlight} in config to disable all auto-formatting',
  "Define custom formatter commands with file extensions in config",
  "Klika Code uses LSP servers for intelligent code analysis",
  "Create {highlight}.ts{/highlight} files in {highlight}.opencode/tool/{/highlight} to define Klika Code tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.opencode/plugin/{/highlight} for Klika Code event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent Klika Code from reading sensitive files",
  "Use {highlight}klika-code run{/highlight} for non-interactive scripting",
  "Use {highlight}klika-code run --continue{/highlight} to resume the last session",
  "Use {highlight}klika-code run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}klika-code serve{/highlight} for headless API access to Klika Code",
  "Use {highlight}klika-code run --attach{/highlight} to connect to a running server",
  "Run {highlight}klika-code upgrade{/highlight} to update to the latest version",
  "Run {highlight}klika-code auth list{/highlight} to see all configured providers",
  "Run {highlight}klika-code agent create{/highlight} for guided agent creation",
  "Use {highlight}/opencode{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}klika-code github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/opencode fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in Klika Code's {highlight}.opencode/themes/{/highlight} directory",
  ...
  "Run {highlight}klika-code debug config{/highlight} to troubleshoot configuration",
  ...
  "Use {highlight}/connect{/highlight} with Klika Code Zen for curated, tested models",
